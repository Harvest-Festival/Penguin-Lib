...
public class Chatter {
    ...
    public static String modify(ITextComponent text, String... formatting) {
        ...
        } else return new TranslationTextComponent(text.getString()).getString();
    }
    ...
    public void draw(MatrixStack matrix, FontRenderer fontRenderer, int x, int y, int color) {
        ...
        for (int i = 0; i < line; i++) {
            ...
            if (text != null) {
                if (this.formatting == null) fontRenderer.draw(matrix, text, x, y + (i * height), color);
                else fontRenderer.draw(matrix, this.formatting + text, x, y + (i * height), color);
            }
        }
        ...
        if (line < maxLines && fordisplay != null) {//If the current line, is less than the length of the lines, And we have less pages than max
            if (script[page][line] != null) {
                ...
                else fontRenderer.draw(matrix, this.formatting + new String(fordisplay), x, y + (line * height), color);
            } else finished = true;
        }
    }

    private String[][] buildScript(FontRenderer fontRenderer, String text) {
        List<ITextProperties> formatted = fontRenderer.getSplitter().splitLines(text, maxWidth, Style.EMPTY);
        String[] original = formatted.stream().map(ITextProperties::getString).toArray(String[]::new);
